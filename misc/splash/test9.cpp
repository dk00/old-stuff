#include<cstdio>
int dx[]={1,0,-1,0},dy[]={0,1,0,-1},bad[]={0,100,75,50,25},
od[6][6]=
{{5,4,3,3,4,5},
{4,2,1,1,2,4},
{3,1,0,0,1,3},
    ...
    for(i=0;i*2+1<n;)
    {
        if(i*2+2>=n || s[i*2+1].score<s[i*2+2].score)
            if(s[i].score>s[i*2+1].score)
                tmp=s[i],s[i]=s[i*2+1],s[i*2+1]=tmp,i=i*2+1;
            else
                break;
        else if(s[i].score>s[i*2+2].score)
            tmp=s[i],s[i]=s[i*2+2],s[i*2+2]=tmp,i=i*2+2;
        else
            break;
    }
}
    ...
    if(n==1)
    {
        for(x=0;x<36;x++)
        {
            ...
            if(tp[1].s[i][j]==4)
            {
                ...
                (tp[0].pn<best.pn && tp[0].cnt+3>best.cnt)))
                    best=tp[0];
            }
        }
        return min;
...
void branch(P root)
{
    if(root.pn>2)
        return;
    int i,j,k,x,y,now;
    static const int bound=7;
    P leaf;
    for(i=root.p1;i<36;i++)
    {
        x=list[i].x,y=list[i].y;
        if(root.s[x][y] && root.s[x][y]<4)
        {
            leaf=root;
            leaf.add(x,y);
            if(leaf.s[x][y])
...
main()
{
    ...
    while(1)
    {
        ...
        snum=clear=0;
        branch(test);
        while(heap.n<10000 && heap.n>1 && best.cnt<1)
            branch(heap.exmin());
        printf("+%d\n",best.cnt+best.pn);
        for(i=0;i<best.pn;i++)
