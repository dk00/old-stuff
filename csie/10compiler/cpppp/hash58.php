<?
$table1=array(
	
);
$table2=array(
	
);
$inv=pack('C*',
	
);
$rd=array(
	0x18	=> 8,
	0x32	=> 20,
	0x58	=> 20
);
$wr=array(
	0x18	=> 8,
	0x30	=> 1,
	0x32	=> 20,
	0x58	=> 20
);
//----------------------------------------------------------------------------
function hashfile($filename,$id)
{
	global $rd,$wr;
	$id=pack('H*',$id);
	$key=key0($id);
	$file=&file_get_contents($filename);
	foreach($rd as $offset => $len)
		$tmp[$offset]=substr($file,$offset,$len);
	foreach($wr as $offset => $len)
	{
		if($len==1)$x=chr(1);
		else $x=pack("x$len");
		replace($file,$x,$offset);
	}
	$tmp[0x58]=hash58($key,$file);
  print bin2hex($tmp[0x58])."\n";
	foreach($tmp as $offset => $dat)
		replace($file,$dat,$offset);
	file_put_contents($filename,$file);
}

//----------------------------------------------------------------------------
function hash58($key,&$file)
{
	//$n=strlen($key);
	$n=64;
	for($i=0;$i<$n;$i++)$key[$i]=chr(ord($key[$i])^0x36);  
  $h0=sha1($key.$file,true);
	for($i=0;$i<$n;$i++)$key[$i]=chr(ord($key[$i])^0x36^0x5c);
	return sha1($key.$h0,true);
}
function key0($id)
{
	global $table1,$table2,$inv;
	$fix=pack('C*',
		0x67,0x23,0xFE,0x30,0x45,0x33,0xF8,0x90,0x99,
		0x21,0x07,0xC1,0xD0,0x12,0xB2,0xA1,0x07,0x81);
	$y='';
	$n=strlen($id);
	for($i=0;$i<$n;$i+=2)
	{
		$a=ord($id[$i]);
		$b=ord($id[$i+1]);    
		$a=$a*$b/gcd($a,$b);
		$a=max($a,1);
		$b=$a&0xff;
		$a>>=8;
		$y.=pack('C',$table1[$a]*0xB5-3);
		$y.=pack('C',$table2[$a]*0xB7+0x49);
		$y.=pack('C',$table1[$b]*0xB5-3);
		$y.=pack('C',$table2[$b]*0xB7+0x49);
	}
	$n=strlen($y);
	for($i=0;$i<$n;$i++)
		$y[$i]=$inv[ord($y[$i])];
	return sha1($fix.$y,true);
}
function replace(&$a,&$b,$pos)
{
	$n=strlen($b);
	for($i=0;$i<$n;$i++)
		$a[$pos+$i]=$b[$i];
}
function gcd($a,$b)
{
	if(!$a || !$b)return 1;
	while(($a%=$b)&&($b%=$a));
	return $a+$b;
}
hashfile('iTunesDB','000A27001D4BD7DA');
?>
