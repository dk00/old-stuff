...
int main(int argc, char **argv)
{
    ...
    const char *PIPTestFile=NULL;
    int nProg=0;
    char *const *ProgFiles=NULL;
#ifndef FILESYS_STUB
    ...
    if(PIPTestFile){
        kernel->currentThread->pipSched(PIPTestFile);
    }
    else if(ProgFiles){
        vector<AddrSpace *>space;
	for(int i=0;i<nProg;i++){
	    Thread *t=new Thread(ProgFiles[i]);
	    t->space=new AddrSpace;
            space.push_back(t->space);
	    t->space->Load(ProgFiles[i]);
	    t->prio=i+2;
	    t->Fork((VoidFunctionPtr)RunUserProg,NULL);
	}
	kernel->currentThread->prio=nProg+2;
	kernel->currentThread->Yield();
        puts("\n[Content of Page Tables]");
        for(int i=0;i<space.size();i++)
            delete space[i];
    }
    // finally, run an initial user program if requested to do so
    else if (userProgName != NULL) {
