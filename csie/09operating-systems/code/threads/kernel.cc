...
int Kernel::gettid(Thread *thread)
{
...
void Kernel::releasetid(int tid)
{
    tidTable[tid]=NULL;
}

int Kernel::getPage(int page)
{
    if(page>=0){
        pageUsed[page]++;
        return page;
    ...
    static int i=-1;
    for(int j=i+1;j!=i;++j%=NumPhysPages){
        if(pageUsed[j]==0){
            pageUsed[j]++;
    ...
    cout << "Not enough memory on simulated machine\n";
    return -1;
}

void Kernel::releasePage(int p)
{
    if(pageUsed[p]>0)
        pageUsed[p]--;
}
...
int Kernel::createThread(void *func,void *arg)
{
    char *name=new char[20];
    Thread *thread = new Thread(name);
    thread->tid=gettid(thread);
    sprintf(name,"thread%d",thread->tid);
    thread->prio=currentThread->prio+1;
    if(func==NULL)
	func=(void *)verySimpleThread;
    thread->Fork((VoidFunctionPtr)func,arg);
    return thread->tid;
}

void Kernel::setPriority(int tid,int prio)
{
    Thread* thread=tidTable[tid];
    thread->prio=prio;
    if(scheduler->readyList->IsEmpty())
	return;
    if(scheduler->readyList->IsInList(thread))
	scheduler->readyList->Remove(thread);
    if(thread!=currentThread && !scheduler->readyList->IsInList(thread))
	scheduler->readyList->Insert(thread);
    if(scheduler->readyList->Front()->prio < currentThread->prio)
	currentThread->Yield();
}
...
int
Kernel::idDistribution(Thread* thread){
    return gettid(thread);
}
