...
Thread::Finish ()
{
	...
	kernel->scheduler->ReadyToRun(notifyList[i]);
    if(tid>=0)
	kernel->releasetid(tid);    
...
void Thread::notify(Thread * t)
{
    notifyList.push_back(t);
}

void Thread::Join(int tID)
{
    if(!kernel->tidTable[tID])
	return;
    kernel->tidTable[tID]->notify(this);
    IntStatus oldLevel=kernel->interrupt->SetLevel(IntOff);
    Sleep(false);
...
Thread::Sleep (bool finishing)
{
	...
	while ((nextThread = kernel->scheduler->FindNextToRun()) == NULL){
	  cout<<"Idle from "<<kernel->stats->totalTicks<<endl;
...
Thread::getPrio(){
  int p=prio;
  for(list<Sem*>::iterator i=lock.begin();i!=lock.end();++i){
    int t=(*i)->getPrio();
    if( t > 0 && t < p)
      p = t;
  }
