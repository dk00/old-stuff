...
static void reset_credit(const struct scheduler *ops, int cpu, s_time_t now)
{
    struct csched_runqueue_data *rqd = RQD(ops, cpu);
    struct list_head *iter;
    int n = 0, n_pcpu = 4, max_weight = 0, sum_creditccc;
    int lb, ub, level;
    list_for_each( iter, &rqd->svc ) {
        struct csched_vcpu * svc = list_entry(iter, struct csched_vcpu, rqd_elem);
        ++n;
        if (svc->credit < svc->usage/16)
          svc->usage = 0;
        else svc->usage = svc->usage - svc->credit + svc->usage/16;
        svc->start_time = now;
    ...
    //printk("%s: n = %d, max_weight = %d\n", __func__, n, max_weight);
    lb = 0;
    ub = CSCHED_CREDIT_INIT*n/n_pcpu + 1;
    while (ub - lb > 0) {
        int diff;
        sum_credit = 0;
        level = (lb + ub)/2;
        list_for_each( iter, &rqd->svc ) {
            ...
            min_credit = 1ll*svc->reserve*n*CSCHED_CREDIT_INIT*max_weight/100/n_pcpu/svc->weight;
            max_credit = 1ll*svc->cap*n*CSCHED_CREDIT_INIT*max_weight/100/n_pcpu/svc->weight;
            if (svc->usage > 0 && svc->usage > min_credit && svc->usage < max_credit)
              max_credit = svc->usage;
            if (level > max_credit)
              svc->credit = max_credit;
            else if (level < min_credit)
              svc->credit = min_credit;
            else
              svc->credit = level;
            sum_credit += svc->credit;
        }
        diff = sum_credit - n*CSCHED_CREDIT_INIT;
        if (diff < -n)
          lb = level + 1;
        else if (diff > n)
          ub = level - 1;
    ...
    printk("%s: credit ratio:", __func__);
    list_for_each( iter, &rqd->svc ) {
        struct csched_vcpu * svc = list_entry(iter, struct csched_vcpu, rqd_elem);
        svc->usage = svc->credit;
        printk(" %d", svc->credit);
    }
    printk("\n");
    //printk("%s: level = %d\n", __func__, level);
...
csched_alloc_vdata(const struct scheduler *ops, struct vcpu *vc, void *dd)
{
    ...
    if ( ! is_idle_vcpu(vc) )
    {
        ...
        svc->reserve = 0;
        svc->cap = 100;
        if (svc->vcpu->domain->domain_id == 1 && svc->vcpu->vcpu_id == 0) {
          svc->reserve = 95;
          printk("d%d v%d: reserve = %d, cap = %d\n", 
              svc->vcpu->domain->domain_id, svc->vcpu->vcpu_id, svc->reserve, svc->cap);
        }
        if (svc->vcpu->domain->domain_id == 2 && svc->vcpu->vcpu_id == 0) {
          svc->cap = 30;
          printk("d%d v%d: reserve = %d, cap = %d\n", 
              svc->vcpu->domain->domain_id, svc->vcpu->vcpu_id, svc->reserve, svc->cap);
        }        
