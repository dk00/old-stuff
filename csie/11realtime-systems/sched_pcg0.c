...
#define DEFAULT_SPEED MICROSECS(500)
#define DEFAULT_REQ MILLISECS(10)
...
inline void pcg_event(const struct scheduler *ops, s_time_t now) {
  struct csched_private priv = CSCHED_PRIV(ops);
  //TODO: Lock
  if (now < priv->next_event) return;
