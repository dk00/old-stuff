 ...
 ****************************************************************************
 * PCG Schduler
 * Adopted from pcg_credit2.c
 *
 * Optimal Dynamic-Priority Real-Time Scheduling Algorithms for Uniform Multiprocessors
 * http://www.computer.org/portal/web/csdl/doi/10.1109/RTSS.2008.35
 */
...
#define TRC_SCHED_TICK        TRC_SCHED_CLASS + 1
#define TRC_SCHED_RUNQ_POS    TRC_SCHED_CLASS + 2
#define TRC_SCHED_ER_BURN TRC_SCHED_CLASS + 3
#define TRC_SCHED_ER_ADD  TRC_SCHED_CLASS + 4
#define TRC_SCHED_TICKLE_CHECK TRC_SCHED_CLASS + 5
#define TRC_SCHED_TICKLE       TRC_SCHED_CLASS + 6
#define TRC_SCHED_ER_RESET TRC_SCHED_CLASS + 7
#define TRC_SCHED_SCHED_TASKLET TRC_SCHED_CLASS + 8
#define TRC_SCHED_UPDATE_LOAD   TRC_SCHED_CLASS + 9
#define TRC_SCHED_RUNQ_ASSIGN   TRC_SCHED_CLASS + 10
#define TRC_SCHED_UPDATE_VCPU_LOAD   TRC_SCHED_CLASS + 11
#define TRC_SCHED_UPDATE_RUNQ_LOAD   TRC_SCHED_CLASS + 12

/* Default weight: How much a new domain starts with */
#define PCG_DEFAULT_WEIGHT       256
/* Min timer: Minimum length a timer will be set, to
 * achieve efficiency */
#define PCG_MIN_TIMER            MICROSECS(500)
...
#define PCG_ER_INIT          MILLISECS(10)
/* Carryover: How much "extra" credit may be carried over after
 * a reset. */
#define PCG_CARRYOVER_MAX        PCG_MIN_TIMER
/* Stickiness: Cross-L2 migration resistance.  Should be less than
 * MIN_TIMER. */
#define PCG_MIGRATE_RESIST       ((opt_migrate_resist)*MICROSECS(1))
/* How much to "compensate" a vcpu for L2 migration */
#define PCG_MIGRATE_COMPENSATION MICROSECS(50)
/* Reset: Value below which credit will be reset. */
#define PCG_ER_RESET         0
/* Max timer: Maximum time a guest can be run for. */
#define PCG_MAX_TIMER            MILLISECS(2)


#define PCG_IDLE_CREDIT                 (-(1<<30))

#define __SCHFLAG_scheduled 1
#define CSFLAG_scheduled (1<<__SCHFLAG_scheduled)
/* CSFLAG_delayed_runq_add: Do we need to add this to the runqueue once it'd done
 * being context switched out?
 * + Set when scheduling out in pcg_schedule() if prev is runnable
 * + Set in pcg_vcpu_wake if it finds CSFLAG_scheduled set
 * + Read in pcg_context_saved().  If set, it adds prev to the runqueue and
 *   clears the bit.
 */
#define __SCHFLAG_delayed_runq_add 2
#define CSFLAG_delayed_runq_add (1<<__SCHFLAG_delayed_runq_add)
 ...
 * in the svc struct. 
 */
#define __SCHFLAG_runq_migrate_request 3
#define CSFLAG_runq_migrate_request (1<<__SCHFLAG_runq_migrate_request)
 ...
 * Useful macros
 */
#define PCG_PRIV(_ops)   \
  ((struct pcg_private *)((_ops)->sched_data))
#define PCG_VCPU(_vcpu)  ((struct pcg_vcpu *) (_vcpu)->sched_priv)
#define PCG_DOM(_dom)    ((struct pcg_dom *) (_dom)->sched_priv)
#define PCG_CPUONLINE(_pool)    \
  (((_pool) == NULL) ? &cpupool_free_cpus : &(_pool)->cpu_valid)
/* CPU to runq_id macro */
#define c2r(_ops, _cpu)     (PCG_PRIV(_ops)->runq_map[(_cpu)])
/* CPU to runqueue struct macro */
#define RQD(_ops, _cpu)     (&PCG_PRIV(_ops)->rqd[c2r(_ops, _cpu)])
...
struct pcg_vcpu {
  ...
  struct pcg_runqueue_data *rqd; /* Up-pointer to the runqueue */

  /* Up-pointers */
  struct pcg_dom *sdom;
  struct vcpu *vcpu;

  int er, weight, cap, reserve, pcpu;
  ...
  struct pcg_runqueue_data *migrate_rqd; /* Pre-determined rqd to which to migrate */
};
/*
 * System-wide private data
 */
struct pcg_private {
  spinlock_t lock, lock_event;
  ...
  struct pcg_vcpu *assign[NR_CPUS], *bind[NR_CPUS];
  
  s_time_t next_event, next_reset;
  int cpu_speed[NR_CPUS];
  int load_window_shift;
};

int n_pcpu, n_dom, __runq_count;
 ...
 */
static /*inline*/ int __vcpu_on_runq(struct pcg_vcpu *svc) {
  return !list_empty(&svc->runq_elem);
}

static /*inline*/ struct pcg_vcpu * __runq_elem(struct list_head *elem) {
  return list_entry(elem, struct pcg_vcpu, runq_elem);
}

static int __runq_insert(struct list_head *runq, struct pcg_vcpu *svc) {
  ...
  BUG_ON(test_bit(__SCHFLAG_scheduled, &svc->flags));
...
static void runq_insert(const struct scheduler *ops, unsigned int cpu, struct pcg_vcpu *svc) {
  ...
  { 
    ...
    d.pos = pos;
    trace_var(TRC_SCHED_RUNQ_POS, 0,
        sizeof(d),
        (unsigned char *)&d);
  }
  return;
...
static inline void __runq_remove(struct pcg_vcpu *svc) {
  BUG_ON(!__vcpu_on_runq(svc));
...
static /*inline*/ void runq_tickle(const struct scheduler *ops, unsigned int cpu, struct pcg_vcpu *new, s_time_t now) {
  int ipid=-1;
  struct pcg_runqueue_data *rqd = RQD(ops, cpu);
  struct pcg_vcpu * cur;
  ...
  cur = PCG_VCPU(per_cpu(schedule_data, cpu).curr);
  ipid = cpu;
  BUG_ON(ipid == -1);
...
#ifndef NDEBUG
static /*inline*/ void __pcg_vcpu_check(struct vcpu *vc) {
  struct pcg_vcpu * const svc = PCG_VCPU(vc);
  struct pcg_dom * const sdom = svc->sdom;

  BUG_ON(svc->vcpu != vc);
  BUG_ON(sdom != PCG_DOM(vc->domain));
  if (sdom) {
...
#define PCG_VCPU_CHECK(_vc)  (__pcg_vcpu_check(_vc))
#else
#define PCG_VCPU_CHECK(_vc)
#endif

static void * pcg_alloc_vdata(const struct scheduler *ops, struct vcpu *vc, void *dd) {
  struct pcg_vcpu *svc;
  /* Allocate per-VCPU info */
  svc = xmalloc(struct pcg_vcpu);
  if (svc == NULL)
    return NULL;
  ...
  if (! is_idle_vcpu(vc)) {
    ...
    svc->cap = 100;
    /* Starting load of 50% */
    svc->avgload = 1ULL << (PCG_PRIV(ops)->load_window_shift - 1);
...
static void __runq_assign(struct pcg_vcpu *svc, struct pcg_runqueue_data *rqd) {
  svc->rqd = rqd;
  ...
  /* TRACE */
  {
    ...
    d.vcpu = svc->vcpu->vcpu_id;
    d.rqi=rqd->id;
    trace_var(TRC_SCHED_RUNQ_ASSIGN, 1,
        sizeof(d),
        (unsigned char *)&d);
  }
}

static void runq_assign(const struct scheduler *ops, struct vcpu *vc) {
  struct pcg_vcpu *svc = vc->sched_priv;
  BUG_ON(svc->rqd != NULL);
  __runq_assign(svc, RQD(ops, vc->processor));
}

static void __runq_deassign(struct pcg_vcpu *svc) {
  BUG_ON(__vcpu_on_runq(svc));
...
static void runq_deassign(const struct scheduler *ops, struct vcpu *vc) {
  struct pcg_vcpu *svc = vc->sched_priv;
...
static void pcg_free_vdata(const struct scheduler *ops, void *priv) {
  struct pcg_vcpu *svc = priv;
  xfree(svc);
}
...
static void pcg_vcpu_sleep(const struct scheduler *ops, struct vcpu *vc) {
  ...
  if (per_cpu(schedule_data, vc->processor).curr == vc)
    ...
    BUG_ON(svc->rqd != RQD(ops, vc->processor));
    __runq_remove(svc);
  } else if (test_bit(__SCHFLAG_delayed_runq_add, &svc->flags))
    clear_bit(__SCHFLAG_delayed_runq_add, &svc->flags);
}
...
static void pcg_vcpu_wake(const struct scheduler *ops, struct vcpu *vc) {
  ...
  if (unlikely (test_bit(__SCHFLAG_scheduled, &svc->flags))) {
    set_bit(__SCHFLAG_delayed_runq_add, &svc->flags);
    goto out;
  }
...
static void pcg_context_saved(const struct scheduler *ops, struct vcpu *vc) {
  struct pcg_vcpu * const svc = PCG_VCPU(vc);
  s_time_t now = NOW();
  vcpu_schedule_lock_irq(vc);
  ...
  clear_bit(__SCHFLAG_scheduled, &svc->flags);
   ...
   * bits free.
   */
  if (test_and_clear_bit(__SCHFLAG_delayed_runq_add, &svc->flags)
...
static int choose_cpu(const struct scheduler *ops, struct vcpu *vc) {
  struct pcg_private *prv = PCG_PRIV(ops);
  int i, min_rqi = -1, new_cpu;
  struct pcg_vcpu *svc = PCG_VCPU(vc);
  ...
  if (!spin_trylock(&prv->lock)) {
    if (test_and_clear_bit(__SCHFLAG_runq_migrate_request, &svc->flags)) {
      d2printk("d%dv%d -\n", svc->vcpu->domain->domain_id, svc->vcpu->vcpu_id);
      clear_bit(__SCHFLAG_runq_migrate_request, &svc->flags);
    }
...
static int pcg_cpu_pick(const struct scheduler *ops, struct vcpu *vc) {
  ...
  if (svc->rqd != NULL && RQD(ops, new_cpu) != svc->rqd)
    runq_deassign(ops, vc);
...
static int pcg_dom_cntl(
  ...
  struct pcg_dom * const sdom = PCG_DOM(d);
  struct pcg_private *prv = PCG_PRIV(ops);
  unsigned long flags;
  if (op->cmd == XEN_DOMCTL_SCHEDOP_getinfo) {
    ...
    if (op->u.credit2.weight != 0) {
      ...
      /* Must hold pcg_priv lock to update sdom, runq lock to
       * update csvcs. */
      ...
      list_for_each (iter, &sdom->vcpu) {
        struct pcg_vcpu *svc = list_entry(iter, struct pcg_vcpu, sdom_elem);

        /* NB: Locking order is important here.  Because we grab this lock here, we
         * must never lock pcg_priv.lock if we're holding a runqueue
         * lock. */
...
static void * pcg_alloc_domdata(const struct scheduler *ops, struct domain *dom) {
  ...
  sdom->dom = dom;
  sdom->weight = PCG_DEFAULT_WEIGHT;
  sdom->nr_vcpus = 0;

  spin_lock_irqsave(&PCG_PRIV(ops)->lock, flags);
  list_add_tail(&sdom->sdom_elem, &PCG_PRIV(ops)->sdom);
  spin_unlock_irqrestore(&PCG_PRIV(ops)->lock, flags);
...
static int pcg_dom_init(const struct scheduler *ops, struct domain *dom) {
  ...
  sdom = pcg_alloc_domdata(ops, dom);
  if (sdom == NULL)
    return -ENOMEM;
...
static void pcg_free_domdata(const struct scheduler *ops, void *data) {
  int flags;
  struct pcg_dom *sdom = data;
...
static void pcg_dom_destroy(const struct scheduler *ops, struct domain *dom) {
  struct pcg_dom *sdom = PCG_DOM(dom);
  BUG_ON(!list_empty(&sdom->vcpu));
  pcg_free_domdata(ops, PCG_DOM(dom));
}

/* How long should we let this vcpu run for? */
static s_time_t pcg_runtime(const struct scheduler *ops, int cpu, struct pcg_vcpu *snext)
{
  return PCG_MAX_TIMER;
}
 ...
 * Find a candidate.
 */
static struct pcg_vcpu * runq_candidate(
    const struct scheduler *ops,
    struct pcg_runqueue_data *rqd,
    struct pcg_vcpu *scurr,
    int cpu, s_time_t now) {
  struct list_head *iter;
  struct pcg_vcpu *snext = NULL;
  struct pcg_private *prv = PCG_PRIV(ops);
  ...
  if (vcpu_runnable(scurr->vcpu))
    ...
    snext = PCG_VCPU(idle_vcpu[cpu]);
  /* TODO: cut
  if (prv->bind[cpu]) {
  }*/
  if (prv->assign[cpu] && vcpu_runnable(prv->assign[cpu]->vcpu)) {
    if (scurr->vcpu == prv->assign[cpu]->vcpu)
      return scurr;
    if (__vcpu_on_runq(prv->assign[cpu]))
      return prv->assign[cpu];
    return PCG_VCPU(idle_vcpu[cpu]);
  }

  list_for_each(iter, &rqd->runq) {
    struct pcg_vcpu * svc = list_entry(iter, struct pcg_vcpu, runq_elem);
    snext = svc;
...
static int reset_er(const struct scheduler *ops);
static s_time_t next_event(const struct scheduler *ops);

/*
 * This function is in the critical path. It is designed to be simple and
 * fast for the common case.
 */
static struct task_slice pcg_schedule(
    const struct scheduler *ops, s_time_t now, bool_t tasklet_work_scheduled) {
  const int cpu = smp_processor_id();
  int flag;
  struct pcg_runqueue_data *rqd;
  struct pcg_vcpu * const scurr = PCG_VCPU(current);
  struct pcg_vcpu *snext = NULL;
  struct task_slice ret;
  struct pcg_private *prv = PCG_PRIV(ops);

  PCG_VCPU_CHECK(current);
  now = NOW();

  d2printk("sc p%d c d%dv%d now %"PRI_stime"\n",
      cpu, scurr->vcpu->domain->domain_id,
      scurr->vcpu->vcpu_id, now);

  BUG_ON(!cpu_isset(cpu, PCG_PRIV(ops)->initialized));
   ...
   */
  if (unlikely((now >= prv->next_event || now >= prv->next_reset)&& spin_trylock(&prv->lock_event))) {
    __runq_count = 0;
    spin_lock_irqsave(&prv->lock, flag);
    if (now >= prv->next_event)
      next_event(ops);
    else
      reset_er(ops);
    spin_unlock_irqrestore(&prv->lock, flag);
    spin_unlock(&prv->lock_event);
    if (n_dom > 0 && now%1000 == 0) {
      printk("event: %d\treset: %d\n", (int)(prv->next_event - now), (int)(prv->next_reset - now));
    }    
  }
  if (tasklet_work_scheduled) {
    trace_var(TRC_SCHED_SCHED_TASKLET, 0, 0,  NULL);
    snext = PCG_VCPU(idle_vcpu[cpu]);
  }
  else
    snext = runq_candidate(ops, rqd, scurr, cpu, now);

  /* If switching from a non-idle runnable vcpu, put it
   * back on the runqueue. */
  if (snext != scurr && !is_idle_vcpu(scurr->vcpu) && vcpu_runnable(current)) {
    set_bit(__SCHFLAG_delayed_runq_add, &scurr->flags);
    //TODO: different speed
    scurr->er -= now - scurr->start_time;
  }
  ...
  if (!is_idle_vcpu(snext->vcpu)) {
    ...
    if (snext != scurr) {
      ...
      set_bit(__SCHFLAG_scheduled, &snext->flags);
    }
   ...
   */
  ret.time = pcg_runtime(ops, cpu, snext);
  ret.task = snext->vcpu;

  PCG_VCPU_CHECK(ret.task);
  return ret;
}

static void pcg_dump_vcpu(struct pcg_vcpu *svc) {
...
static void pcg_dump_pcpu(const struct scheduler *ops, int cpu) {
  ...
  /* current VCPU */
  svc = PCG_VCPU(per_cpu(schedule_data, cpu).curr);
  if (svc) {
    printk("\trun: ");
    pcg_dump_vcpu(svc);
  }
...
static void pcg_dump(const struct scheduler *ops) {
  struct list_head *iter_sdom, *iter_svc;
  struct pcg_private *prv = PCG_PRIV(ops);
  int i, loop;

  printk("Active queues: %d\n"
      "\tdefault-weight     = %d\n", cpus_weight(prv->active_queues), PCG_DEFAULT_WEIGHT);
  for_each_cpu_mask(i, prv->active_queues) {
    s_time_t fraction;
  ...
  list_for_each(iter_sdom, &prv->sdom) {
    struct pcg_dom *sdom;
    sdom = list_entry(iter_sdom, struct pcg_dom, sdom_elem);

    printk("\tDomain: %d w %d v %d\n\t", sdom->dom->domain_id, sdom->weight, sdom->nr_vcpus);

    list_for_each(iter_svc, &sdom->vcpu) {
      struct pcg_vcpu *svc;
      svc = list_entry(iter_svc, struct pcg_vcpu, sdom_elem);
...
static void activate_runqueue(struct pcg_private *prv, int rqi) {
  struct pcg_runqueue_data *rqd;
  rqd = prv->rqd + rqi;
...
static void deactivate_runqueue(struct pcg_private *prv, int rqi) {
  struct pcg_runqueue_data *rqd;
...
static void init_pcpu(const struct scheduler *ops, int cpu) {
  ...
  ++n_pcpu;
  prv->cpu_speed[cpu] = 1; //TODO: get real CPU speed
  prv->assign[cpu] = NULL;
  prv->bind[cpu] = NULL;
  rqi = 0;
  rqd = prv->rqd + rqi;
...
static void * pcg_alloc_pdata(const struct scheduler *ops, int cpu) {
...
static void pcg_free_pdata(const struct scheduler *ops, void *pcpu, int cpu) {
  unsigned long flags;
  struct pcg_private *prv = PCG_PRIV(ops);
  struct pcg_runqueue_data *rqd;
  int rqi;
...
static int pcg_cpu_starting(int cpu) {
...
static int cpu_sched_callback(struct notifier_block *nfb, unsigned long action, void *hcpu) {
  unsigned int cpu = (unsigned long)hcpu;
  int rc = 0;

  switch (action) {
    case CPU_STARTING:
      pcg_cpu_starting(cpu);
      break;
    default:
      break;
  }
...
static struct notifier_block cpu_sched_nfb = {
  .notifier_call = cpu_sched_callback
};

static int pcg_init(struct scheduler *ops) {
  int i;
  struct pcg_private *prv;
  printk("Initializing PCG scheduler\n" \
  ...
  prv = xmalloc(struct pcg_private);
  if (prv == NULL) return -ENOMEM;
  memset(prv, 0, sizeof(*prv));
  ops->sched_data = prv;
  spin_lock_init(&prv->lock);
  spin_lock_init(&prv->lock_event);
  INIT_LIST_HEAD(&prv->sdom);

  register_cpu_notifier(&cpu_sched_nfb);
...
static void pcg_deinit(const struct scheduler *ops) {
  struct pcg_private *prv;

  prv = PCG_PRIV(ops);
  if (prv != NULL) xfree(prv);
}

static int reset_er(const struct scheduler *ops) {
  //XXX: aquire lock(s) before calling this func
  int i;
  struct pcg_runqueue_data *rqd = RQD(ops, 0);
  struct list_head *iter;
  struct pcg_private *prv = PCG_PRIV(ops);
  s_time_t to_reset = 0;  
  list_for_each (iter, &rqd->svc) {
    struct pcg_vcpu *svc = list_entry(iter, struct pcg_vcpu, rqd_elem);
    if (__vcpu_on_runq(svc))
      svc->er = PCG_ER_INIT;
    else
      svc->er = PCG_MIN_TIMER;
    to_reset += svc->er;
  } //TODO: reservation, cap
  for (i = 0; i < n_pcpu; ++i)
    prv->bind[i] = NULL;
  to_reset /= n_pcpu; //TODO: different speed CPU
  prv->next_reset = NOW() + to_reset + PCG_MIN_TIMER;
  return 0;
}

/*static void cut(int cpu, struct vcpu *vc) {
...
static s_time_t next_event(const struct scheduler *ops) {
  ...
  //TODO: cut
  for (i = 0; i < n_pcpu; ++i) {
    if (prv->bind[i]) continue;
    list_for_each (iter, &rqd->svc) {
      struct pcg_vcpu *svc = list_entry(iter, struct pcg_vcpu, rqd_elem);
      if (!vcpu_runnable(svc->vcpu))
        continue;
      if (svc->pcpu >= 0) continue;
      if (prv->assign[i] == NULL || svc->er > prv->assign[i]->er)
        prv->assign[i] = svc;
    }
    if (prv->assign[i])
      prv->assign[i]->pcpu = i;      
  ...
  now = NOW();
  to_next = prv->next_reset - now;
  for (i = 0; i < n_pcpu; ++i) {
    if (prv->assign[i]) { // finish / Bottom
      if (prv->assign[i]->er < to_next) //TODO: different speed
        to_next = prv->assign[i]->er;
    }
    list_for_each (iter, &rqd->svc) {
      struct pcg_vcpu *svc = list_entry(iter, struct pcg_vcpu, rqd_elem);
      if (!vcpu_runnable(svc->vcpu))
      ...
      if (prv->next_reset - now - svc->er < to_next)
        to_next = prv->next_reset - now - svc->er; //Ceil
    }
  }
  return now + to_next;
}
