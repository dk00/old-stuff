...
#define RR_DEFAULT_WEIGHT       256
/* Min timer: Minimum length a timer will be set, to
 * achieve efficiency */
#define RR_MIN_TIMER            MICROSECS(500)
...
#define RR_CREDIT_INIT          MILLISECS(10)
/* Carryover: How much "extra" credit may be carried over after
 * a reset. */
#define RR_CARRYOVER_MAX        RR_MIN_TIMER
/* Stickiness: Cross-L2 migration resistance.  Should be less than
 * MIN_TIMER. */
#define RR_MIGRATE_RESIST       ((opt_migrate_resist)*MICROSECS(1))
/* How much to "compensate" a vcpu for L2 migration */
#define RR_MIGRATE_COMPENSATION MICROSECS(50)
/* Reset: Value below which credit will be reset. */
#define RR_CREDIT_RESET         0
/* Max timer: Maximum time a guest can be run for. */
#define RR_MAX_TIMER            MILLISECS(2)


#define RR_IDLE_CREDIT                 (-(1<<30))
 ...
 * + Set when scheduling out in rr_schedule() if prev is runnable
 * + Set in rr_vcpu_wake if it finds CSFLAG_scheduled set
 * + Read in rr_context_saved().  If set, it adds prev to the runqueue and
 *   clears the bit.
 */
 ...
 */
#define RR_PRIV(_ops)   \
  ((struct rr_private *)((_ops)->sched_data))
#define RR_VCPU(_vcpu)  ((struct rr_vcpu *) (_vcpu)->sched_priv)
#define RR_DOM(_dom)    ((struct rr_dom *) (_dom)->sched_priv)
#define RR_CPUONLINE(_pool)    \
  (((_pool) == NULL) ? &cpupool_free_cpus : &(_pool)->cpu_valid)
/* CPU to runq_id macro */
#define c2r(_ops, _cpu)     (RR_PRIV(_ops)->runq_map[(_cpu)])
/* CPU to runqueue struct macro */
#define RQD(_ops, _cpu)     (&RR_PRIV(_ops)->rqd[c2r(_ops, _cpu)])
...
struct rr_private {
  ...
  struct rr_runqueue_data rqd[NR_CPUS];
...
struct rr_vcpu {
  ...
  struct rr_runqueue_data *rqd; /* Up-pointer to the runqueue */
  ...
  struct rr_runqueue_data *migrate_rqd; /* Pre-determined rqd to which to migrate */
};
 ...
 */
static /*inline*/ int __vcpu_on_runq(struct rr_vcpu *svc) {
  return !list_empty(&svc->runq_elem);
}

static /*inline*/ struct rr_vcpu * __runq_elem(struct list_head *elem) {
  return list_entry(elem, struct rr_vcpu, runq_elem);
}

static int __runq_insert(struct list_head *runq, struct rr_vcpu *svc) {
...
static void runq_insert(const struct scheduler *ops, unsigned int cpu, struct rr_vcpu *svc) {
...
static inline void __runq_remove(struct rr_vcpu *svc) {
  BUG_ON(!__vcpu_on_runq(svc));
...
static /*inline*/ void runq_tickle(const struct scheduler *ops, unsigned int cpu, struct rr_vcpu *new, s_time_t now) {
  int ipid=-1;
  struct rr_runqueue_data *rqd = RQD(ops, cpu);
  struct rr_vcpu * cur;
  ...
  cur = RR_VCPU(per_cpu(schedule_data, cpu).curr);
  ipid = cpu;
  BUG_ON(ipid == -1);
...
#ifndef NDEBUG
static /*inline*/ void __rr_vcpu_check(struct vcpu *vc) {
  struct rr_vcpu * const svc = RR_VCPU(vc);
  struct rr_dom * const sdom = svc->sdom;

  BUG_ON(svc->vcpu != vc);
  BUG_ON(sdom != RR_DOM(vc->domain));
  if (sdom) {
...
#define RR_VCPU_CHECK(_vc)  (__rr_vcpu_check(_vc))
#else
#define RR_VCPU_CHECK(_vc)
#endif

static void * rr_alloc_vdata(const struct scheduler *ops, struct vcpu *vc, void *dd) {
  struct rr_vcpu *svc;
  /* Allocate per-VCPU info */
  svc = xmalloc(struct rr_vcpu);
  if (svc == NULL)
    return NULL;
  ...
  if (! is_idle_vcpu(vc)) {
    ...
    svc->cap = 100;
    /* Starting load of 50% */
    svc->avgload = 1ULL << (RR_PRIV(ops)->load_window_shift - 1);
...
static void __runq_assign(struct rr_vcpu *svc, struct rr_runqueue_data *rqd) {
  svc->rqd = rqd;
...
static void runq_assign(const struct scheduler *ops, struct vcpu *vc) {
  struct rr_vcpu *svc = vc->sched_priv;
  BUG_ON(svc->rqd != NULL);
  __runq_assign(svc, RQD(ops, vc->processor));
}

static void __runq_deassign(struct rr_vcpu *svc) {
  BUG_ON(__vcpu_on_runq(svc));
...
static void rr_vcpu_insert(const struct scheduler *ops, struct vcpu *vc) {
  struct rr_vcpu *svc = vc->sched_priv;
...
static void rr_free_vdata(const struct scheduler *ops, void *priv) {
  struct rr_vcpu *svc = priv;
  xfree(svc);
}

static void rr_vcpu_remove(const struct scheduler *ops, struct vcpu *vc) {
  struct rr_vcpu * const svc = RR_VCPU(vc);
  struct rr_dom * const sdom = svc->sdom;
...
static void rr_vcpu_sleep(const struct scheduler *ops, struct vcpu *vc) {
  struct rr_vcpu * const svc = RR_VCPU(vc);
  BUG_ON(is_idle_vcpu(vc));
...
static void rr_vcpu_wake(const struct scheduler *ops, struct vcpu *vc) {
  struct rr_vcpu * const svc = RR_VCPU(vc);
  s_time_t now = 0;
...
static void rr_context_saved(const struct scheduler *ops, struct vcpu *vc) {
  struct rr_vcpu * const svc = RR_VCPU(vc);
  s_time_t now = NOW();
  vcpu_schedule_lock_irq(vc);
...
static int choose_cpu(const struct scheduler *ops, struct vcpu *vc) {
  struct rr_private *prv = RR_PRIV(ops);
  int i, min_rqi = -1, new_cpu;
  struct rr_vcpu *svc = RR_VCPU(vc);
...
static int rr_cpu_pick(const struct scheduler *ops, struct vcpu *vc) {
  struct rr_vcpu * const svc = RR_VCPU(vc);
  int new_cpu;
...
static int rr_dom_cntl(
    const struct scheduler *ops,
    struct domain *d,
    struct xen_domctl_scheduler_op *op) {
  struct rr_dom * const sdom = RR_DOM(d);
  struct rr_private *prv = RR_PRIV(ops);
  unsigned long flags;
  ...
  if (op->cmd == XEN_DOMCTL_SCHEDOP_getinfo) {
    ...
    if (op->u.credit2.weight != 0) {
      ...
      /* Must hold rr_priv lock to update sdom, runq lock to
       * update csvcs. */
      ...
      list_for_each (iter, &sdom->vcpu) {
        struct rr_vcpu *svc = list_entry(iter, struct rr_vcpu, sdom_elem);

        /* NB: Locking order is important here.  Because we grab this lock here, we
         * must never lock rr_priv.lock if we're holding a runqueue
         * lock. */
...
static void * rr_alloc_domdata(const struct scheduler *ops, struct domain *dom) {
  struct rr_dom *sdom;
  int flags;

  sdom = xmalloc(struct rr_dom);
  if (sdom == NULL)
    return NULL;
  ...
  sdom->dom = dom;
  sdom->weight = RR_DEFAULT_WEIGHT;
  sdom->nr_vcpus = 0;

  spin_lock_irqsave(&RR_PRIV(ops)->lock, flags);
  list_add_tail(&sdom->sdom_elem, &RR_PRIV(ops)->sdom);
  spin_unlock_irqrestore(&RR_PRIV(ops)->lock, flags);

  return (void *)sdom;
}

static int rr_dom_init(const struct scheduler *ops, struct domain *dom) {
  struct rr_dom *sdom;
...
static void rr_free_domdata(const struct scheduler *ops, void *data) {
  int flags;
  struct rr_dom *sdom = data;
...
static void rr_dom_destroy(const struct scheduler *ops, struct domain *dom) {
  struct rr_dom *sdom = RR_DOM(dom);
  BUG_ON(!list_empty(&sdom->vcpu));
  rr_free_domdata(ops, RR_DOM(dom));
}

/* How long should we let this vcpu run for? */
static s_time_t rr_runtime(const struct scheduler *ops, int cpu, struct rr_vcpu *snext)
{
  return RR_MAX_TIMER;
}
 ...
 * Find a candidate.
 */
static struct rr_vcpu * runq_candidate(
    struct rr_runqueue_data *rqd,
    struct rr_vcpu *scurr,
    int cpu, s_time_t now) {
  struct list_head *iter;
  struct rr_vcpu *snext = NULL;

  /* Default to current if runnable, idle otherwise */
  if (vcpu_runnable(scurr->vcpu))
    snext = scurr;
  else
    snext = RR_VCPU(idle_vcpu[cpu]);

  list_for_each(iter, &rqd->runq) {
    struct rr_vcpu * svc = list_entry(iter, struct rr_vcpu, runq_elem);
    snext = svc;
 ...
 */
static struct task_slice rr_schedule(
    const struct scheduler *ops, s_time_t now, bool_t tasklet_work_scheduled) {
  const int cpu = smp_processor_id();
  struct rr_runqueue_data *rqd;
  struct rr_vcpu * const scurr = RR_VCPU(current);
  struct rr_vcpu *snext = NULL;
  struct task_slice ret;

  RR_VCPU_CHECK(current);

  d2printk("sc p%d c d%dv%d now %"PRI_stime"\n",
      cpu, scurr->vcpu->domain->domain_id,
      scurr->vcpu->vcpu_id, now);

  BUG_ON(!cpu_isset(cpu, RR_PRIV(ops)->initialized));
   ...
   */
  ret.time = rr_runtime(ops, cpu, snext);
  ret.task = snext->vcpu;

  RR_VCPU_CHECK(ret.task);
  return ret;
}

static void rr_dump_vcpu(struct rr_vcpu *svc) {
...
static void rr_dump_pcpu(const struct scheduler *ops, int cpu) {
  ...
  /* current VCPU */
  svc = RR_VCPU(per_cpu(schedule_data, cpu).curr);
  if (svc) {
    printk("\trun: ");
    rr_dump_vcpu(svc);
  }
...
static void rr_dump(const struct scheduler *ops) {
  struct list_head *iter_sdom, *iter_svc;
  struct rr_private *prv = RR_PRIV(ops);
  int i, loop;

  printk("Active queues: %d\n"
      "\tdefault-weight     = %d\n", cpus_weight(prv->active_queues), RR_DEFAULT_WEIGHT);
  for_each_cpu_mask(i, prv->active_queues) {
    s_time_t fraction;
  ...
  list_for_each(iter_sdom, &prv->sdom) {
    struct rr_dom *sdom;
    sdom = list_entry(iter_sdom, struct rr_dom, sdom_elem);

    printk("\tDomain: %d w %d v %d\n\t", sdom->dom->domain_id, sdom->weight, sdom->nr_vcpus);

    list_for_each(iter_svc, &sdom->vcpu) {
      struct rr_vcpu *svc;
      svc = list_entry(iter_svc, struct rr_vcpu, sdom_elem);
...
static void activate_runqueue(struct rr_private *prv, int rqi) {
  struct rr_runqueue_data *rqd;
  rqd = prv->rqd + rqi;
...
static void deactivate_runqueue(struct rr_private *prv, int rqi) {
  struct rr_runqueue_data *rqd;
...
static void * rr_alloc_pdata(const struct scheduler *ops, int cpu) {
...
static void rr_free_pdata(const struct scheduler *ops, void *pcpu, int cpu) {
  unsigned long flags;
  struct rr_private *prv = RR_PRIV(ops);
  struct rr_runqueue_data *rqd;
  int rqi;
...
static int rr_cpu_starting(int cpu) {
...
static int rr_init(struct scheduler *ops) {
  int i;
  struct rr_private *prv;
...
static void rr_deinit(const struct scheduler *ops) {
  struct rr_private *prv;
...
static struct rr_private _rr_priv;

const struct scheduler sched_rr_def = {
  .name           = "RR Scheduler",
  .opt_name       = "rr",
  .sched_id       = 11,
  .sched_data     = &_rr_priv,

  .init_domain    = rr_dom_init,
  .destroy_domain = rr_dom_destroy,

  .insert_vcpu    = rr_vcpu_insert,
  .remove_vcpu    = rr_vcpu_remove,
  ...
  .pick_cpu       = rr_cpu_pick,
  .do_schedule    = rr_schedule,
  .context_saved  = rr_context_saved,

  .dump_cpu_state = rr_dump_pcpu,
  .dump_settings  = rr_dump,
  .init           = rr_init,
  .deinit         = rr_deinit,
  .alloc_vdata    = rr_alloc_vdata,
  .free_vdata     = rr_free_vdata,
  .alloc_pdata    = rr_alloc_pdata,
  .free_pdata     = rr_free_pdata,
  .alloc_domdata  = rr_alloc_domdata,
  .free_domdata   = rr_free_domdata,
};
