...
print_user_counts(unum)
{
  ...
  sprintf(chatbuf,
    "¡ó Åwªï¥úÁ{¡i¶§¥ú¯ù§{¡j¡A¥Ø«e¶}¤F [1;31m%d[0m ¶¡¥]´[", roomc + 1);
  send_to_unum(unum, chatbuf);
  sprintf(chatbuf, "¡ó ¦@¦³ [1;36m%d[0m ¤H¨ÓÂ\\Àsªù°}", userc + 1);
  if (suserc)
    sprintf(chatbuf + strlen(chatbuf), " [%d ¤H¦bºNºN¯ù]", suserc);
  send_to_unum(unum, chatbuf);
  return 0;
}
