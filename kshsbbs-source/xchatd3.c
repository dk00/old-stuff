...
#define CHAT_PIDFILE    "/bbs1/home/bbs/run/chat.pid"
#define CHAT_LOGFILE    "/bbs1/home/bbs/run/chat.log"
#define SOCK_QLEN       5
  ...
  if (room = user->room)
  {
    ...
    if (--room->occupants > 0)
    {
      ...
      case EXIT_LOSTCONN:
        ...
        sprintf(chatbuf, "¡» %s ¦¨¤FÂ_¤Fªº¤ô²yÅo", chatid);
        break;
  ...
  char *msg;
{
  ...
  {
    send_to_user(cu, "¡° ¤w¸g¦³¤H±¶¨¬¥ýµnÅo¡A´«¤@­Ó§a:)", 0);
    return;
  }
  ...
  char *msg;
{
  ...
  if (room == NULL)
  {
    ...
    (void) strcpy(room->topic, "³o¬O·s²á¤Ñ«Ç³é^^*..");
  ...
  else
  {
    ...
    {
      send_to_user(cuser, "¡° ¤º¦³¬yª]¡A«D½Ð²ö¤J", 0);
      return 0;
    }
  }
  ...
  ChatUser *cuser;
{
  ...
  sprintf(chatbuf,
    "¡ó Åwªï¥úÁ{¡i¬õ¼Ó²á¤Ñ§{¡j¡A¥Ø«e¶}¤F [1;31m%d[m ¶¡¥]´[ [Ctrl-G ¬Ý»²§U»¡©ú]", roomc);
  send_to_user(cuser, chatbuf, 0);
...
{
  {"aluba", "§â","§ì°_¨Óªü¾|¤Ú¨ì¦º...."},
  {"bearhug", "¼ö±¡ªº¾Ö©ê", ""},
  {"bless", "¯¬ºÖ", "¤ß·Q¨Æ¦¨"},
  {"bow", "²¦°`²¦·qªº¦V", "Áù°`"},
  {"bye",       "§tµÛ²´²\\¡A±aµÛ·L¯ºªº¦V","»¡ÙTÙT"},
  {"call",      "¦V»a¤Ñ§o³Û~~~","~~~§A¦b¨º¸Ì§r"},
  {"caress", "¼¾ºN", ""},
  {"cringe", "¦V", "¨õ°`©}½¥¡A·n§À¤^¼¦"},
  {"cry", "¦V", "Àz°Þ¤j­ú"},
  {"comfort", "·Å¨¥¦w¼¢", ""},
  {"clap", "¦V", "¼ö¯P¹ª´x"},
  {"claw","§ì§ì","ªº¤â¤â...."},
  {"congr1","¹ï","°Û:¡u§A¯u¥¿¤F¤£°_¡A¯u¥¿¤F¤£°_"},
  {"congr2","¹ï","°Û:¡u¨S¤°»ò¤F¤£°_¡A¨S¤°»ò¤F¤£°_"},
  {"dance", "©Ô¤F", "ªº¤â½¡½¡°_»R"},
  {"destroy","°á¤F¤U©G¤å...¹ï","¬I¥X¤F¤õ²y³N~~~D-E-S-T-R-O-Y"},
  {"dogleg", "¹ï", "ª¯»L"},
  {"drivel", "¹ïµÛ", "¬y¤f¤ô"},
  {"envy","»¡:¡u","¡A§Ú¸r¼}§A³é~~~¡v"},
  {"eye","¦V","²r©ß´A²´..*^Q^*"},
  {"forgive","­ì½Ì","¹ï§Ú°µªº¤@¤Á¿ù»~"},
  {"giggle", "¹ïµÛ", "¶Ì¶Ìªº§b¯º"},
  {"goodbye","¹ï","»¡¦A¨£Åo...¦³ªÅ¦A¨Ó"},
  {"grin", "¹ï", "ÅS¥X¨¸´cªº¯º®e"},
  {"growl", "¹ï", "©H­ý¤£¤w"},
  {"hand", "¸ò", "´¤¤â"},
  {"hiak", "¦V","¦l¯º,¨Ãµo¥Xhiak...hiak...hiak...ªºÁn­µ@_@"},
  {"hospital","§â","°e¶i¶¯¤¤¹ï­±ªº«n¤sÂå°|"},
  {"hug", "»´»´¦a¾Ö©ê", ""},
  {"kick", "§â", "½ðªº¦º¥h¬¡¨Ó"},
  {"kiss", "»´§k", "ªºÁyÀU"},
  {"laugh", "¤jÁn¼J¯º", ""},
  ...
  {"superwater","®³°_¤ô±í´Â","¼â¤U¥h°Õ~~~"},
  {"thank", "¦V", "¹DÁÂ"},
  {"tickle", "©B¼T!©B¼T!·k", "ªºÄo"},
  {"water","®³°_¤ô²y¦V","¥Î¤Oªº¯{¤U¥h"},
  {"wave", "¹ïµÛ", "«÷©Rªº·n¤â"},
  {"wink", "¹ï", "¯«¯µªº¯w¯w²´·ú"},
  {"zap", "¹ï", "ºÆ¨gªº§ðÀ»"},
  {NULL, NULL, NULL}
...
ChatAction condition_data[] =
{
  ...
  {"balian","­A¥ÕÄê¡A·í¥Õ·ö...",NULL},
  {"blush", "Áy³£¬õ¤F", NULL},
  {"cough", "«y¤F´XÁn", NULL},
  {"gold","¯u¦æ¡A¯u¦æ...¥X°ê¤ñÁÉ..>_<..±o«a­x¡A®³ª÷µP..",NULL},
  {"happy", "r-o-O-m....Å¥¤F¯u²n¡I", NULL},
  {"kshs","¨º¶¡¤õ¨®¯¸«eªº¾Ç®Õ³é...",NULL},
  {"kghs","³á...¶¯¤k³é..­IµÛ¬õ¦âªº®Ñ¥]³é",NULL},
  {"luck", "«z¡IºÖ®ð°Õ¡I", NULL},
  {"macarn","ºÆ¨gªº¸õµÛº¿¥d½¬®R...",NULL},
  {"miou","ØpØpØp~~~®³°_CatGªº¿ß¤ö...~",NULL},
...
start_daemon()
{
  ...
  fd = open(CHAT_PIDFILE, O_WRONLY | O_CREAT, 0644);
  if (fd >= 0)
  {
    sprintf(buf, "%5d\n", getpid());
    write(fd, buf, 6);
