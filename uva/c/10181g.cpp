...
int rev[LEN*LEN][2],pre[LEN*LEN][LEN][LEN],map[LEN][LEN];
int ans[100],c1,c2,anscnt;
bool flag;
...
int IDA_DFS(int bound,int depth,int nscore,int nx,int ny)
{
	...
	for(k=0;k<4;k++)
	{
		...
		if(k+ans[anscnt-1]==3)
			continue;
		tmp=map[tx][ty],map[nx][ny]=map[tx][ty],map[tx][ty]=0;
		...
		if(tmp<next)
			next=tmp;
        map[tx][ty]=map[nx][ny],map[nx][ny]=0;
		anscnt--;
	}
	return next;
}
	...
	for(t=0;t<cnt;t++)
	{
		...
		for(a=0,k=x+y;a<LEN;a++)
			for(b=0;b<LEN;b++)
				for(c=0;c<LEN;c++)
					for(d=0;d<LEN;d++)
						if(((a*LEN+b)<(c*LEN+d))&&map[a][b]>map[c][d])
							k++;
		map[x][y]=0;
		if(score==0)
		{
